...
export function logEvent(entry: Omit<AuditLogEntry, 'id' | 'timestamp' | 'createdAt' | 'updatedAt'>, empresaId: string): void {
  try {
...
export function queryLogs(empresaId: string, filters?: {
  ...
  try {
    ...
    if (filters) {
      ...
      if (filters.to) {
        // Ajuste para incluir todo o dia se for apenas uma data (YYYY-MM-DD)
