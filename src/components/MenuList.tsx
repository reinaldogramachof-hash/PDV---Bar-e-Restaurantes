      ...
      <AnimatePresence mode="popLayout">
        {filteredProducts.map((product) => (
          <motion.button
            ...
            className={`group relative flex flex-col items-start p-5 rounded-3xl border text-left transition-all duration-300
              ${isDark ? 'bg-[#1C1C1E] border-[#2C2C2E] hover:border-[#E85D75]/40' : 'bg-white border-gray-100 hover:border-[#E85D75]/40 shadow-sm hover:shadow-xl shadow-gray-200/20'}
            `}
          >
            {/* Price Tag */}
            <div className={`absolute top-4 right-4 px-3 py-1.5 rounded-xl font-black text-[10px] tracking-tight shadow-sm
              ${isDark ? 'bg-white/5 text-white' : 'bg-[#E85D75] text-white'}`}>
              R$ {product.price.toFixed(2)}
            </div>

            {/* Icon/Emoji Container */}
            <div className={`w-12 h-12 rounded-2xl flex items-center justify-center text-2xl mb-4 transition-all duration-500 group-hover:scale-110
            ...
            <div className="space-y-1 w-full flex-1">
              <h4 className="text-xs font-black uppercase tracking-tight group-hover:text-[#E85D75] transition-colors leading-tight line-clamp-2">
                {product.name}
              </h4>
