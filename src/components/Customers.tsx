    ...
    <div className="flex flex-col h-full gap-8 animate-in fade-in duration-700">
      <div className="flex flex-col lg:flex-row justify-between lg:items-end gap-6">
        <div className="space-y-1">
          ...
          <p className={`text-xs font-bold uppercase tracking-widest ${isDark ? 'text-[#3A3A3C]' : 'text-gray-400'}`}>Base de dados e fidelidade</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
        {mockCustomers.filter(c => c.name.toLowerCase().includes(searchTerm.toLowerCase())).map(customer => (
          <motion.div 
            whileHover={{ y: -5 }}
            key={customer.id} 
            className={`p-6 rounded-[2.5rem] border transition-all ${isDark ? 'bg-[#1C1C1E] border-[#2C2C2E]' : 'bg-white border-gray-100 shadow-sm hover:shadow-xl shadow-gray-200/20'}`}
          >
            <div className="flex justify-between items-start mb-6">
              ...
              <div className="flex gap-1">
                 <button className="p-2 rounded-lg hover:bg-current/5 opacity-40 hover:opacity-100 transition-all"><Edit3 className="w-4 h-4" /></button>
                 <button className="p-2 rounded-lg hover:bg-red-500/10 text-red-500 opacity-40 hover:opacity-100 transition-all"><Trash2 className="w-4 h-4" /></button>
              </div>
            </div>
            ...
            <div className="mt-8 pt-6 border-t border-dashed border-current/10 grid grid-cols-2 gap-4">
              <div><p className="text-[8px] font-black uppercase tracking-widest opacity-30 mb-1">Total Gasto</p><p className="text-sm font-black text-[#E85D75]">R$ {customer.totalSpent.toFixed(2)}</p></div>
              <div className="text-right">
                <p className="text-[8px] font-black uppercase tracking-widest opacity-30 mb-1">Pontos Fidelidade</p>
                <span className="px-2 py-1 bg-purple-500/10 text-purple-500 rounded-full text-[10px] font-black">{customer.loyaltyPoints} pts</span>
              </div>
            </div>
          </motion.div>
        ))}
