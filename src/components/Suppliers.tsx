        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-2 gap-4">
          {filteredSuppliers.map(supplier => (
            <motion.div
              key={supplier.id}
              layout
              className={`p-5 rounded-panel border transition-all duration-300 group hover:border-[var(--color-accent)]/40 ${isDark ? 'bg-[var(--color-surface)] border-[var(--color-border)]' : 'bg-surface-light border-border-light shadow-xl shadow-gray-200/20'}`}
            >
              <div className="flex justify-between items-start mb-4">
                <div className="flex items-center gap-3">
                  <div className={`w-10 h-10 rounded-panel flex items-center justify-center relative ${isDark ? 'bg-surface-light/5' : 'bg-elevated-light'}`}>
                    <Truck className="w-5 h-5 text-[var(--color-accent)]" />
                    <div className="absolute -top-1 -right-1 flex gap-0.5">
                       {[...Array(Math.min(3, supplier.rating || 0))].map((_, i) => <Star key={i} className="w-2.5 h-2.5 fill-amber-500 text-warning" />)}
                    </div>
                  </div>
                  <div>
                    <h3 className="text-sm font-semibold truncate max-w-[200px]">{supplier.companyName}</h3>
                    <div className="flex items-center gap-2 mt-1">
                       <span className={`text-[10px] font-medium px-2 py-0.5 rounded-control ${isDark ? 'bg-surface-light/5 text-muted-light' : 'bg-elevated-light text-muted-light'}`}>{supplier.category}</span>
                       {supplier.deliveryPerformance >= 95 && <span className="flex items-center gap-1 text-[10px] font-medium text-success"><Check className="w-3 h-3" /> Premium</span>}
                    </div>
                  </div>
                </div>
                <div className="flex gap-1.5 opacity-0 group-hover:opacity-100 transition-all">
                   <button onClick={() => handleOpenModal(supplier)} className={`p-1.5 rounded-control transition-all ${isDark ? 'bg-surface-light/5 hover:bg-surface-light/10' : 'bg-elevated-light hover:bg-elevated-light'}`}><Edit3 className="w-3.5 h-3.5 opacity-40" /></button>
                   <button onClick={() => deleteSupplier(supplier.id)} className="p-1.5 rounded-control bg-danger/10 text-danger hover:bg-danger/10 transition-all"><Trash2 className="w-3.5 h-3.5" /></button>
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div className="space-y-2">
                  <div className="flex items-center gap-3">
                     <div className={`w-7 h-7 rounded-control flex items-center justify-center shrink-0 ${isDark ? 'bg-surface-light/5' : 'bg-elevated-light'}`}><User className="w-3 h-3 opacity-30" /></div>
                  ...
                  <div className="flex items-center gap-3">
                     <div className={`w-7 h-7 rounded-control flex items-center justify-center shrink-0 ${isDark ? 'bg-surface-light/5' : 'bg-elevated-light'}`}><Phone className="w-3 h-3 opacity-30" /></div>
                     <div><span className="text-[10px] text-muted block">Telefone</span><span className="text-xs font-medium">{supplier.phone}</span></div>
                  </div>
                  <div className="flex items-center gap-3">
                     <div className={`w-7 h-7 rounded-control flex items-center justify-center shrink-0 ${isDark ? 'bg-surface-light/5' : 'bg-elevated-light'}`}><MapPin className="w-3 h-3 opacity-30" /></div>
                ...
                <div className={`p-4 rounded-control flex flex-col justify-between border ${isDark ? 'bg-black/20 border-border' : 'bg-elevated-light border-border-light'}`}>
                   <div>
                      ...
                      <div className="h-1.5 bg-current/10 rounded-full overflow-hidden">
                         <motion.div initial={{ width: 0 }} animate={{ width: `${supplier.deliveryPerformance}%` }} transition={{ duration: 1.5, ease: "easeOut" }} className={`h-full rounded-full ${supplier.deliveryPerformance >= 90 ? 'bg-success' : 'bg-warning'}`} />
                      </div>
                   </div>
              ...
              <div className="mt-4 pt-3 border-t border-current/5 flex justify-between items-center">
                 ...
                 {supplier.observations && <div className="flex items-center gap-1 text-[10px] font-medium text-warning/60 bg-warning/10 px-2.5 py-1 rounded-full"><AlertCircle className="w-3 h-3" /> Ver Notas</div>}
              </div>
            </motion.div>
          ))}
        </div>
        ...
        <div className={`rounded-section border overflow-hidden ${isDark ? 'bg-[var(--color-surface)] border-[var(--color-border)]' : 'bg-surface-light border-border-light shadow-2xl shadow-gray-200/10'}`}>
          <div className="overflow-x-auto">
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className={`text-[9px] uppercase font-semibold tracking-[0.2em] border-b ${isDark ? 'bg-surface-light/5 border-border text-muted' : 'bg-elevated-light border-border-light text-muted-light'}`}>
                  ...
                  <tr key={supplier.id} className="group hover:bg-current/[0.01] transition-all">
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-4">
                        ...
                        <div className="flex flex-col">
                           <span className="text-[12px] font-semibold uppercase tracking-tight leading-tight">{supplier.companyName}</span>
                           <span className="text-[9px] font-bold opacity-30 mt-0.5">{supplier.document || 'Sem CNPJ'}</span>
                        </div>
                      </div>
                    </td>
                    <td className="px-4 py-3">
                       <span className={`text-[8px] font-semibold  px-2.5 py-1 rounded-lg ${isDark ? 'bg-surface-light/5 text-muted-light' : 'bg-elevated-light text-muted-light'}`}>{supplier.category}</span>
                    </td>
                    <td className="px-4 py-3">
                       <div className="flex flex-col">
                    ...
                    <td className="px-4 py-3">
                       <div className="flex items-center gap-2">
                          ...
                          <span className="text-[10px] font-semibold opacity-40 ">{supplier.paymentTerms || 'Boleto'}</span>
                       </div>
                    </td>
                    <td className="px-4 py-3 text-right">
                       <div className="flex flex-col items-end gap-1.5">
                          <span className={`text-[11px] font-semibold ${supplier.deliveryPerformance >= 90 ? 'text-success' : 'text-warning'}`}>{supplier.deliveryPerformance}%</span>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 lg:p-8">
            ...
            >
                  ...
                  {activeTab === 'geral' ? (
                    <div className="space-y-4 animate-in fade-in slide-in-from-left-4 duration-300">
                      <div className="grid grid-cols-1 lg:grid-cols-2 gap-3">
                        <div className="space-y-1.5">
                          ...
                          <input required value={formData.companyName} onChange={e => setFormData({ ...formData, companyName: e.target.value })} className={`w-full h-10 px-3 rounded-control border outline-none text-sm focus:ring-2 focus:ring-[var(--color-accent)]/20 ${isDark ? 'bg-[var(--color-app-base)] border-[var(--color-border)]' : 'bg-elevated-light border-border-light'}`} />
                        </div>
                        <div className="space-y-1.5">
                          <label className="text-xs text-muted ml-1">CNPJ / CPF</label>
                          <input value={formData.document} onChange={e => setFormData({ ...formData, document: e.target.value })} className={`w-full h-10 px-3 rounded-control border outline-none text-sm focus:ring-2 focus:ring-[var(--color-accent)]/20 ${isDark ? 'bg-[var(--color-app-base)] border-[var(--color-border)]' : 'bg-elevated-light border-border-light'}`} />
                        </div>
                      </div>
                      ...
                      <div className="grid grid-cols-1 lg:grid-cols-2 gap-3">
                        <div className="space-y-1.5">
                          ...
                          <input value={formData.contactName} onChange={e => setFormData({ ...formData, contactName: e.target.value })} className={`w-full h-10 px-3 rounded-control border outline-none text-sm focus:ring-2 focus:ring-[var(--color-accent)]/20 ${isDark ? 'bg-[var(--color-app-base)] border-[var(--color-border)]' : 'bg-elevated-light border-border-light'}`} />
                        </div>
                        <div className="space-y-1.5">
                          <label className="text-xs text-muted ml-1">Categoria</label>
                          <select value={formData.category} onChange={e => setFormData({ ...formData, category: e.target.value })} className={`w-full h-10 px-3 rounded-control border outline-none text-sm appearance-none focus:ring-2 focus:ring-[var(--color-accent)]/20 ${isDark ? 'bg-[var(--color-app-base)] border-[var(--color-border)] text-white' : 'bg-elevated-light border-border-light text-text-light'}`}>
                      ...
                      <div className="grid grid-cols-1 lg:grid-cols-2 gap-3">
                        <div className="space-y-1.5">
                          ...
                          <input value={formData.phone} onChange={e => setFormData({ ...formData, phone: e.target.value })} className={`w-full h-10 px-3 rounded-control border outline-none text-sm focus:ring-2 focus:ring-[var(--color-accent)]/20 ${isDark ? 'bg-[var(--color-app-base)] border-[var(--color-border)]' : 'bg-elevated-light border-border-light'}`} />
                        </div>
                        <div className="space-y-1.5">
                          <label className="text-xs text-muted ml-1">E-mail Comercial</label>
                          <input type="email" value={formData.email} onChange={e => setFormData({ ...formData, email: e.target.value })} className={`w-full h-10 px-3 rounded-control border outline-none text-sm focus:ring-2 focus:ring-[var(--color-accent)]/20 ${isDark ? 'bg-[var(--color-app-base)] border-[var(--color-border)]' : 'bg-elevated-light border-border-light'}`} />
                        </div>
                      </div>
                    </div>
                  ) : (
                    <div className="space-y-4 animate-in fade-in slide-in-from-right-4 duration-300">
                      <div className="space-y-1.5">
                        ...
                        <textarea value={formData.address} onChange={e => setFormData({ ...formData, address: e.target.value })} rows={2} className={`w-full px-3 py-2 rounded-control border outline-none text-sm focus:ring-2 focus:ring-[var(--color-accent)]/20 ${isDark ? 'bg-[var(--color-app-base)] border-[var(--color-border)]' : 'bg-elevated-light border-border-light'}`} />
                      </div>
                      ...
                      <div className="grid grid-cols-1 lg:grid-cols-2 gap-3">
                        ...
                        <div className="space-y-1.5">
                          ...
                          <div className="flex gap-2 h-10 px-3 rounded-control border items-center bg-[var(--color-app-base)] border-[var(--color-border)]">
                             {[1, 2, 3, 4, 5].map(r => (
              ...
              <div className="px-5 py-4 border-t border-current/5 bg-black/5 bg-surface-light/5 flex gap-3">
                <button type="button" onClick={() => setIsModalOpen(false)} className={`flex-1 h-10 rounded-panel text-xs font-medium transition-all ${isDark ? 'bg-surface-light/5 hover:bg-surface-light/10' : 'bg-elevated-light hover:bg-border-light'}`}>Descartar</button>
