  ...
  return (
    <div className="max-w-5xl mx-auto space-y-12 animate-in fade-in slide-in-from-bottom-4 duration-700 pb-24">
      ...
      <div className="flex flex-col md:flex-row items-center gap-8 border-b border-dashed border-current/10 pb-12">
        <div className="w-24 h-24 rounded-[2rem] bg-blue-500/10 flex items-center justify-center flex-shrink-0">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        {sections.map((section, idx) => (
          <div 
            key={idx}
            className={`p-8 rounded-[2.5rem] border space-y-6 relative overflow-hidden group transition-all hover:scale-[1.02]
              ${isDark ? 'bg-[#1C1C1E] border-[#2C2C2E]' : 'bg-white border-gray-100 shadow-xl shadow-gray-200/20'}`}
          >
      ...
      {/* Terms and Details */}
      <div className={`rounded-[3rem] border p-10 space-y-10 relative overflow-hidden
        ${isDark ? 'bg-[#1C1C1E] border-[#2C2C2E]' : 'bg-white border-gray-100 shadow-2xl shadow-gray-200/30'}`}>
        
        <div className="flex flex-col md:flex-row gap-12 relative z-10">
          <div className="flex-1 space-y-6">
            <div className="flex items-center gap-3">
              <FileText className="w-5 h-5 text-[#E85D75]" />
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              {terms.map((term, idx) => (
                <div key={idx} className="flex gap-4 p-4 rounded-2xl bg-current/5">
                  <div className="w-1.5 h-1.5 rounded-full bg-[#E85D75] mt-1.5 flex-shrink-0" />
          ...
          <div className={`md:w-72 p-8 rounded-[2rem] space-y-6 border border-dashed flex flex-col justify-between
