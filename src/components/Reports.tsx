    ...
    <div className="flex flex-col min-h-full gap-8 animate-in fade-in duration-700 pb-12">
      ...
      <div className="flex flex-col lg:flex-row justify-between lg:items-end gap-6">
        <div className="space-y-1">
          <h2 className="text-3xl font-semibold  leading-none">BI & Financeiro</h2>
          <p className="text-xs font-bold  opacity-40">Gestão Consolidada de Fluxo e Operação</p>
        </div>

        <div className="flex flex-wrap gap-3">
          <div className="flex p-1.5 gap-1.5 rounded-panel bg-black/5 dark:bg-white/5 border border-white/10 backdrop-blur-md">
            {(['hoje', 'semana', 'mes', 'total'] as Period[]).map((p) => (
              <button 
                key={p} 
                onClick={() => setPeriod(p)}
                className={`px-4 py-2 rounded-control text-xs font-semibold  transition-all ${period === p ? 'bg-white dark:bg-[var(--color-border)] shadow-xl text-[var(--color-accent)]' : 'opacity-30 hover:opacity-100'}`}
              >
                {p}
              </button>
            ))}
          </div>
          <button onClick={() => setIsExpenseModalOpen(true)} className="px-6 py-3 bg-[var(--color-accent)] text-white rounded-panel flex items-center gap-3 transition-all">
             <Plus className="w-4 h-4" />
             <span className="text-xs font-semibold ">Lançar Despesa</span>
          </button>
        </div>
      </div>
      ...
      <div className="flex-shrink-0 flex p-1.5 gap-1.5 rounded-panel bg-black/5 dark:bg-white/5 border border-white/10 w-fit backdrop-blur-md">
        {(['dashboard', 'fluxo', 'vendas', 'produtos', 'atendentes'] as Tab[]).map((t) => (
          <button 
            ...
            className={`px-6 py-3 rounded-control text-xs font-semibold  transition-all duration-300 relative ${activeTab === t ? 'bg-white dark:bg-[var(--color-border)] shadow-xl text-[var(--color-accent)]' : 'text-gray-500 hover:text-gray-900 dark:hover:text-white hover:bg-white/5 opacity-50'}`}
          >
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                {[
                  ...
                  <div key={i} className={`p-8 rounded-panel border ${isDark ? 'bg-[var(--color-surface)] border-[var(--color-border)]' : 'bg-white border-gray-100 shadow-xl shadow-gray-200/20'}`}>
                    <div className={`w-12 h-12 rounded-panel ${kpi.bg} ${kpi.color} flex items-center justify-center mb-4`}>
                      <kpi.icon className="w-6 h-6" />
                    </div>
                    <p className="text-xs font-semibold  opacity-30 mb-1">{kpi.label}</p>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                <div className={`lg:col-span-2 p-10 rounded-section border ${isDark ? 'bg-[var(--color-surface)] border-[var(--color-border)]' : 'bg-white border-gray-100 shadow-2xl'}`}>
                  <div className="flex justify-between items-center mb-8">
                    <h3 className="text-xl font-semibold ">Estrutura de Gastos</h3>
                    <PieChart className="w-5 h-5 opacity-20" />
                  </div>
                  <div className="space-y-6">
                    {Object.entries(expenseCategoryStats).length === 0 ? (
                      <div className="py-12 text-center opacity-30 text-xs font-bold ">Nenhuma despesa lançada no período</div>
                    ) : (
                ...
                <div className={`p-10 rounded-section border ${isDark ? 'bg-gradient-to-br from-[var(--color-accent)]/10 to-transparent border-[var(--color-accent)]/20' : 'bg-pink-50/30 border-pink-100'}`}>
                   <div className="flex items-center gap-4 mb-8">
                      ...
                      <h3 className="text-xl font-semibold ">DRE Resumido</h3>
                   </div>
                   <div className="space-y-4">
                      <div className="flex justify-between items-center p-3 border-b border-current/5">
                         <span className="text-xs font-semibold opacity-40">Faturamento</span>
                         <span className="text-sm font-semibold">R$ {totalSalesAmount.toLocaleString('pt-BR')}</span>
                      </div>
                      <div className="flex justify-between items-center p-3 border-b border-current/5">
                         <span className="text-xs font-semibold opacity-40">CMV (Custo Produtos)</span>
                         <span className="text-sm font-semibold text-red-400">-(R$ {cmvTotal.toLocaleString('pt-BR', { minimumFractionDigits: 2 })})</span>
                      </div>
                      <div className="flex justify-between items-center p-3 border-b border-current/5">
                         <span className="text-xs font-semibold opacity-40">Despesas (Insumos/Gerais)</span>
                         <span className="text-sm font-semibold text-red-400">-(R$ {totalExpensesAmount.toLocaleString('pt-BR', { minimumFractionDigits: 2 })})</span>
                      </div>
                      <div className={`flex justify-between items-center p-4 rounded-panel mt-4 ${netProfit >= 0 ? 'bg-emerald-500/10 text-emerald-500' : 'bg-red-500/10 text-red-500'}`}>
                         <span className="text-xs font-semibold">Resultado Final</span>
                         <span className="text-lg font-semibold">R$ {netProfit.toLocaleString('pt-BR')}</span>
                      </div>
                   </div>
                </div>
              </div>
                      ...
                      {filteredExpenses.reverse().map(e => (
                        <tr key={e.id} className="group hover:bg-current/[0.01] transition-all">
                          ...
                          <td className="px-10 py-6">
                             <span className="px-3 py-1 rounded-full text-xs font-semibold  bg-white/5 border border-current/10">
                               {e.category}
                             </span>
                          </td>
                          <td className="px-10 py-6 text-xs font-semibold">{e.description}</td>
                          <td className="px-10 py-6">
                             <span className={`px-3 py-1 rounded-full text-xs font-semibold  ${e.status === 'pago' ? 'bg-emerald-500/10 text-emerald-500' : 'bg-amber-500/10 text-amber-500'}`}>
                               {e.status}
                             </span>
                          </td>
                          <td className="px-10 py-6 text-right font-semibold text-red-400">R$ {e.amount.toFixed(2)}</td>
                          <td className="px-10 py-6 text-right">
                             <button onClick={() => deleteExpense(e.id)} className="p-2 rounded-control text-red-500 hover:bg-red-500/10 opacity-0 group-hover:opacity-100 transition-all">
                  ...
                  <tbody className="divide-y divide-current/5">
                    {salesData.map(o => (
                      <tr key={o.id} className="group hover:bg-current/[0.01] transition-all">
                        ...
                        <td className="px-10 py-6 font-semibold text-xs">#{o.id.slice(-6)}</td>
                        <td className="px-10 py-6">
                           <span className={`px-3 py-1 rounded-full text-xs font-semibold  ${isDark ? 'bg-white/5' : 'bg-gray-100'}`}>
                             {o.mode === 'mesa' ? `Mesa ${o.tableNumber}` : 'Balcão'}
                           </span>
                        </td>
                        <td className="px-10 py-6 text-xs font-bold opacity-60 ">{o.payments.map(p => p.method).join(' + ')}</td>
                        <td className="px-10 py-6 text-right font-semibold text-[var(--color-accent)]">R$ {o.total.toFixed(2)}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
                ...
                {productRanking.map((p, i) => (
                  <div key={p.name} className={`p-8 rounded-panel border relative overflow-hidden group ${isDark ? 'bg-[var(--color-surface)] border-[var(--color-border)]' : 'bg-white border-gray-100 shadow-xl'}`}>
                    <div className="absolute top-0 right-0 p-4">
                       <span className={`text-4xl font-semibold opacity-5 ${i < 3 ? 'text-[var(--color-accent)]' : ''}`}>{i + 1}º</span>
                    </div>
                    <div className="space-y-1 mb-6">
                       <p className="text-xs font-semibold  opacity-30">{p.category}</p>
                       <h4 className="text-lg font-semibold  truncate pr-8">{p.name}</h4>
                    </div>
                    <div className="grid grid-cols-2 gap-4">
                       <div className="p-4 rounded-panel bg-black/5 dark:bg-white/5 border border-current/5">
                          <p className="text-xs font-semibold opacity-30 mb-1">Volume</p>
                          <p className="text-xl font-semibold">{p.qty}</p>
                       </div>
                       <div className="p-4 rounded-panel bg-[var(--color-accent)]/5 border border-[var(--color-accent)]/10">
                          <p className="text-xs font-semibold text-[var(--color-accent)]/60 mb-1">Receita</p>
                          <p className="text-xl font-semibold text-[var(--color-accent)]">R$ {p.revenue.toFixed(0)}</p>
                       </div>
                    </div>
                  </div>
                ))}
                ...
                {waiterStats.map((w, i) => (
                  <div key={w.id} className={`p-8 rounded-panel border flex flex-col md:flex-row justify-between items-center gap-8 ${isDark ? 'bg-[var(--color-surface)] border-[var(--color-border)]' : 'bg-white border-gray-100 shadow-xl'}`}>
                     <div className="flex items-center gap-6">
                        <div className="w-16 h-16 rounded-panel bg-gradient-to-br from-[var(--color-accent)] to-orange-400 flex items-center justify-center text-white text-xl font-semibold">
                           {w.name[0]}
                        </div>
                        <div>
                           <h4 className="text-xl font-semibold ">{w.name}</h4>
                           <p className="text-xs font-semibold  opacity-40">{w.ordersCount} Pedidos Finalizados</p>
                        </div>
                     </div>
                     
                     <div className="flex gap-8 items-center">
                        <div className="text-center">
                           <p className="text-xs font-semibold opacity-30 mb-1">Vendas Totais</p>
                           <p className="text-xl font-semibold">R$ {w.revenue.toFixed(2)}</p>
                        </div>
                        <div className="h-10 w-px bg-current/10" />
                        <div className="text-center">
                           <p className="text-xs font-semibold text-emerald-500 mb-1">Comissão Acumulada</p>
                           <p className="text-xl font-semibold text-emerald-500">R$ {w.commission.toFixed(2)}</p>
                        </div>
                        <button className="w-12 h-12 rounded-panel bg-[var(--color-accent)]/10 text-[var(--color-accent)] flex items-center justify-center hover:bg-[var(--color-accent)] hover:text-white transition-all"><ChevronRight className="w-6 h-6" /></button>
                     </div>
                  </div>
                ))}
             </motion.div>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            <motion.div initial={{ scale: 0.9, opacity: 0, y: 20 }} animate={{ scale: 1, opacity: 1, y: 0 }} exit={{ scale: 0.9, opacity: 0, y: 20 }} className={`relative w-full max-w-lg p-8 rounded-section border shadow-2xl ${isDark ? 'bg-[var(--color-surface)] border-[var(--color-border)]' : 'bg-white border-gray-100'}`}>
               <div className="flex items-center gap-4 mb-8">
                  <motion.div 
                    layout
                    className={`w-12 h-12 rounded-panel flex items-center justify-center transition-colors duration-500
                  ...
                  <div>
                     <h3 className="text-2xl font-semibold ">Novo Lançamento</h3>
                     <p className="text-xs font-bold opacity-40">Registre uma saída financeira</p>
                  </div>
               </div>

               <form onSubmit={handleAddExpense} className="space-y-8">
                  <div className="space-y-3">
                     <label className="text-xs font-semibold opacity-40 ml-2">Escolha a Categoria</label>
                     <div className="grid grid-cols-3 sm:grid-cols-4 gap-2">
                        {EXPENSE_CATEGORIES.map(cat => (
                           <button
                              ...
                              className={`py-3 px-2 rounded-panel border text-xs font-semibold  transition-all flex flex-col items-center gap-2
                  ...
                  <div className="grid grid-cols-1 gap-6">
                     <div className="space-y-2">
                        <label className="text-xs font-semibold opacity-40 ml-2">Status do Pagamento</label>
                        <div className="flex gap-2">
                           {['pago', 'pendente'].map(status => (
                              <button
                                 ...
                                 className={`flex-1 py-3 rounded-panel text-xs font-semibold  border transition-all
                     ...
                     <div className="space-y-2">
                        <label className="text-xs font-semibold opacity-40 ml-2">Descrição / Favorecido</label>
                        <input 
                           required
                           type="text" 
                           ...
                           className={`w-full px-5 py-4 rounded-panel border bg-transparent text-sm font-bold focus:outline-none focus:ring-2 focus:ring-[var(--color-accent)]/20 ${isDark ? 'border-[var(--color-border)]' : 'border-gray-100'}`}
                        />
                     </div>

                     <div className="space-y-2">
                        <label className="text-xs font-semibold opacity-40 ml-2">Valor Total</label>
                        <div className="relative">
                           ...
                           <input 
                              ...
                              placeholder="0,00"
                              className={`w-full pl-12 pr-5 py-4 rounded-panel border bg-transparent text-xl font-semibold focus:outline-none focus:ring-2 focus:ring-[var(--color-accent)]/20 ${isDark ? 'border-[var(--color-border)]' : 'border-gray-100'}`}
                           />
                        </div>
                     </div>
                  </div>
                  ...
                  <div className="flex gap-3 pt-4">
                     <button type="button" onClick={() => setIsExpenseModalOpen(false)} className="flex-1 py-4 rounded-panel font-semibold  text-xs opacity-40 hover:opacity-100 transition-all">Cancelar</button>
                     <button type="submit" className="flex-[2] py-4 rounded-panel bg-[var(--color-accent)] text-white font-semibold  text-xs transition-all">Confirmar Lançamento</button>
                  </div>
               </form>
            </motion.div>
          </div>
